-- ============================================================
-- v007_seed_dra_Andrea.sql
-- Carga inicial da profissional Dra. Andrea Mota.
...
BEGIN
  ...
  WHERE NOT EXISTS (
    ...
    WHERE name = 'Consultório Dra. Andrea Mota'
  )
    ...
    prompt_config = '{
      ...
      "layers": {
        ...
        "professional": {"override": null, "extra_rules": [
          "A Dra. Andrea Mota é especialista em Prótese Dentária.",
          ...
          "A Dra. Andrea integra o corpo docente do curso de residência em Reabilitação Oral da ABO-BA."
        ]},
        "persona": {"override": null, "extra_rules": [
          "Seu nome é DeIA, inspirado no apelido Déa/Deia da Dra. Andrea.",
  ...
  UPDATE clinicas.whatsapp_instances SET
    ...
    provider_config      = jsonb_build_object('instance_id', 'Dra Andrea Mota'),
    status               = 'disconnected'
  WHERE professional_id = v_prof_id;

  RAISE NOTICE 'Seed: Dra. Andrea Mota registrada (professional_id: %)', v_prof_id;
END;
$$;
