#!/usr/bin/env python3
"""
Script para adicionar documentos de consultórios odontológicos à knowledge base.
...
e determinar qual schema do Supabase será usado ao indexar. Os documentos são armazenados
em schemas separados SEM coluna professional_id (schema-isolation conforme ADR de Bruce).

Uso:
    # Adicionar documentos para tenant específico:
    PROFESSIONAL_ID=profissional-demo python knowledge_base_consultorio.py
    
    # Para novo tenant, primeiro adicionar config em NOVOS_DOCUMENTOS
"""
...
DEFAULT_PROFESSIONAL_ID = os.getenv("PROFESSIONAL_ID", None)

if not DEFAULT_PROFESSIONAL_ID:
    print("❌ Error: PROFESSIONAL_ID environment variable required")
    print("   Example: PROFESSIONAL_ID=profissional-demo python knowledge_base_consultorio.py")
    sys.exit(1)

# Documentos genéricos para consultórios odontológicos (aplicáveis a qualquer profissional)
# NOTE: professional_id here is for JSON organization and schema routing only.
# When indexed to Supabase, documents go to tenant-specific schemas WITHOUT 
# a professional_id column (schema-isolation pattern per Bruce's ADR).
# 
# IMPORTANT: These are TEMPLATE documents with placeholder text.
# Replace professional-specific content (names, phones, addresses) before use.
NOVOS_DOCUMENTOS = [
    # ============================================
    # AGENDAMENTO E CONSULTAS (genérico - aplicável a qualquer profissional)
    # ============================================
    {
        "id": "consult_001_agendamento",
        "professional_id": DEFAULT_PROFESSIONAL_ID,  # Will be updated per professional
        "categoria": "agendamento",
        "titulo": "Como Agendar uma Consulta",
        "conteudo": "Para agendar uma consulta, você pode: 1) Enviar mensagem pelo WhatsApp informando o procedimento desejado e datas de preferência; 2) Ligar para o telefone do consultório. Ao agendar, informe: nome completo, telefone, procedimento desejado e preferência de horário. [Preencha horário de atendimento específico do profissional]",
    ...
    },
    {
        ...
        "conteudo": "Na primeira consulta, o profissional realizará uma avaliação completa que inclui: exame clínico, análise de exames complementares (se necessário), verificação de problemas e elaboração de um plano de tratamento personalizado. Traga documentos pessoais, cartão do convênio (se tiver) e lista de medicamentos que utiliza. A consulta de avaliação dura aproximadamente 30 minutos.",
        "metadata": {"fonte": "manual", "atualizado": "2026-03-04", "relevancia": "muito_alto"}
    },
    {
    ...
    # ============================================
    {
        ...
        "conteudo": "[Preencha com especialidades oferecidas pelo profissional: Clínica Geral, Prótese Dentária, Reabilitação Oral, Estética Dental, Ortodontia, Implantodontia, etc.]",
        "metadata": {"fonte": "manual", "atualizado": "2026-03-04", "relevancia": "muito_alto"}
    },
    {
        "id": "proc_002_limpeza",
    ...
    # ============================================
    {
        ...
        "conteudo": "Para verificar se seu convênio é aceito, informe o nome do plano que confirmaremos. Pacientes com convênio devem trazer a carteirinha e documento de identidade. Alguns procedimentos podem ter coparticipação. Procedimentos estéticos geralmente não são cobertos por convênios. [Preencha com convênios específicos aceitos pelo profissional]",
        "metadata": {"fonte": "manual", "atualizado": "2026-03-04", "relevancia": "alto"}
    },
    {
